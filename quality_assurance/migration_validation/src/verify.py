...
def main():
    ...
    checks['root_contains_only_registered_modules_and_agent_instructions'] = {p.name for p in root.iterdir() if p.is_file()} <= {'AGENTS.md'} and actual_cores == {m['core_module'] for m in registry}
    ...
    checks['historical_demand_cards_unchanged'] = read(runs[-1] / 'demand_cards.json') == read(live / 'demand_cards.json')
    checks['fresh_analysis_uses_current_gate'] = (read(runs[0] / 'demand_cards.json') == []
        and read(runs[0] / 'manifest.json')['counts']['current_problem_signals'] == 1
        and read(runs[0] / 'search_plan.json') == [])
    ...
    missing_links = []
    for relative in ['AGENTS.md', 'project_governance/project_state/docs/TASK_STATE.md', 'project_governance/user_guide/docs/START_HERE.md', 'project_governance/workspace_registry/docs/MODULE_INDEX.md']:
        file = root / relative
